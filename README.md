...
本集成提供了删除实体历史数据的功能，可以通过以下方式使用：

#### 1. 自动删除（推荐）

在**参数设置**中启用"删除设备时同时删除实体历史数据"选项：
- 启用后，删除缴费号设备时会自动删除该设备所有实体的历史数据
- 删除主集成时会自动删除所有缴费号的所有实体历史数据
- 历史数据包括历史记录和统计数据
...
**`china_southern_power_grid_stat.purge_device_data`**
- 删除指定缴费号设备的所有实体历史数据
- 参数：`device_id` - 设备ID

**`china_southern_power_grid_stat.purge_all_data`**
- 删除此南方电网账号下所有缴费号的所有实体历史数据
- 无需参数

可以在开发者工具 > 操作（服务）中调用这些服务。

**注意：** 删除历史数据是不可逆操作，请谨慎使用。
